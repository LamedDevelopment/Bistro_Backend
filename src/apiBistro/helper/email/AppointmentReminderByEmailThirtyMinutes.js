...
const { getUpdateAppointmentThirtyMinutesDAO } = require("../../appointmentFollowUp/dao/appointmentFollowUpDAO");


async function SendMailsThirtyMinutes(GetDataSendMailsDto) {
    ...
    try {
        ...
        const fromAppointment = 'Email de 30 Minutos'
        const Token = await generarJWTSendEmailRemember(appointmentID, fromAppointment, time)
        
        const GetAppointmentFollowUpThirtyMinutesDao = await getUpdateAppointmentThirtyMinutesDAO(appointmentID, dateService, timeService, horaActual)
